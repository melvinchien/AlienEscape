...
(function($, undefined) {
    ...
    fnTest = /xyz/.test(function() {/**@nosideeffects*/
        xyz;
    }) ? /\bparent\b/ : /.*/;
...
(function($, undefined) {
    ...
    me.audio = (function() {
		 ...
		 *          me.audio.play("engine", true, 1.0); // play the "gameover_sfx"
		 *          audio clip and call myFunc when finished
		 *          me.audio.play("gameover_sfx", false, 0.75, myFunc);
		 */

        function _play_audio_enable(sound_id, vol, loop, callback) {
            // console.log("play!!");
            var soundclip = get(sound_id.toLowerCase());

            soundclip.loop = loop || false;
            soundclip.volume = typeof vol !== 'undefined' ? vol : 1.0;
            soundclip.play();
		 ...
		 */
        obj.playTrack = function(sound_id,vol) {
            if (sound_enable) {
                ...
                if (current_track) {
                    current_track.volume = typeof vol !== 'undefined' ? vol : 1.0;
        ...
        /**
		 * get the raw html5 audio object<br>
		 *
		 * @name me.audio#getid
		 * @public
		 * @function
		 * @param {String} sound_id
		 * @example me.audio.getid("sfx1");
		 */
        obj.getid = function(sound_id) {
            return current_track = get(sound_id.toLowerCase());
        };
                ...
                for(var t=0, l=obj.touches.length; t<l; t++) {
                    ...
                    for (var i = handlers.length, handler; i--, handler = handlers[i];) {
                        ...
                        if (handler.floating===false) {
                            var v = {
                                x: x + vpos.x - map_pos.x,
                                y: y + vpos.y - map_pos.y
                            };
                        } else {
                            var v = {
                                x: x,
        ...
        obj.registerMouseEvent = function(eventType, rect, callback, floating) {
            ...
            switch (eventType) {
                ...
                case 'touchend':
                    ...
                    obj.mouse.handlers[eventType].push({
                        rect:rect||null,
                        cb:callback,
                        floating:floating===true?true:false
                        });
                    break;
                default :
